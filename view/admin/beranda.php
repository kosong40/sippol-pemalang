<div class="container-fluid">
        <br>
            <center><img src="../aset/img/pemalang.jpg" alt=""></center>
        <br>
    </div>
    <div class="conteiner"><h4 align="center"><b>----- Layanan -----</b></h4></div>
    <br>
    <div class="row">
        <div class="col-sm-3">
            <img src="../aset/img/no_img.svg" class="img-rounded img-responsive" alt="">
            <h4 align="center">Izin Mendirikan Bangunan</h4>
        </div>
        <div class="col-sm-3">
            <img src="../aset/img/no_img.svg" class="img-rounded img-responsive" alt="">    
            <h4 align="center">Izin Reklame</h4>
        </div>
        <div class="col-sm-3">
            <a href="?t=<?php echo $_SESSION["sesi_id"] ?>&p=iumk&n=2""><img src="../aset/img/no_img.svg" class="img-rounded img-responsive" alt=""></a>
            <a href="?t=<?php echo $_SESSION["sesi_id"] ?>&p=iumk&n=2"><h4 align="center">Izin Usaha Mikro dan Kecil</h4></a>
            
        </div>
        <div class="col-sm-3">
            <img src="../aset/img/no_img.svg" class="img-rounded img-responsive" alt="">    
            <h4 align="center">Izin Pariwisata</h4>
        </div>
    </div>